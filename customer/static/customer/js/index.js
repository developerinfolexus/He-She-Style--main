...
function setupHoverSubcategoryNavigation() {
    ...
    sliders.forEach(wrapper => {
        ...
        // Helper to get stride dynamically
        const getItemWidth = () => {
            if (items.length > 0) {
                const item = items[0];
        ...
        const getVisibleItems = () => {
            // Always show maximum of 7 items visible at once
            if (!viewport) return maxVisible;
            if (window.innerWidth < 992) return 2; // Mobile fallback

            // On desktop, we dictate the width, so we always show maxVisible
            return maxVisible;
        };
        ...
        const updateSlider = () => {
            ...
            // Constraint viewport to ensure overflow
            // --- MODIFICATION: Make size dynamic based on actual item width ---
            const itemsToShowVisible = getVisibleItems();
            const currentItemWidth = getItemWidth();
            viewport.style.width = `${itemsToShowVisible * currentItemWidth}px`;
            // --- END MODIFICATION ---
            ...
            if (totalItems <= itemsToShow) {
                prevBtn.style.display = 'none';
                nextBtn.style.display = 'none';
                track.style.transform = 'none'; // Reset if no slide needed
            } else {
                prevBtn.style.display = 'flex';
                nextBtn.style.display = 'flex';

                // Opacity/Interaction for start/end
                if (currentIndex <= 0) {
        ...
        newNext.addEventListener('click', (e) => {
            ...
            // Advance by visible page or single item? 
            // Previous code did 1 item, header code did page. Let's do page for faster nav, or 1 for precision.
            // Let's match header.html logic: slide by visible items
